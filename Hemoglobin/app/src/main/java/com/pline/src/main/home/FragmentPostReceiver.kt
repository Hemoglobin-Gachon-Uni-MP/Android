    ...
    override fun onGetFeedListSuccess(response: ArrayList<FeedListResult>) {
        ...
        for (i in 0.. response.size - 1){
            if (response[i].isReceiver == "T"){
                var list = response[i]
                var post = Post(true, R.drawable.ic_my_page_unselected, list.nickname, list.date, list.context, list.commentCnt, list.abo, list.rh, list.feedId, list.location, list.userId )
